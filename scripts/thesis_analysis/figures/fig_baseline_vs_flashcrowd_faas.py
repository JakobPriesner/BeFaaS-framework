#!/usr/bin/env python3
"""Generate delta_auth_baseline_vs_flashcrowd_faas.pdf for fig:delta-auth-phases-faas.

FaaS: ΔAuth P99 Baseline vs Flash Crowd per HW tier.
"""
import sys, os
...
ARCH = 'faas'
PROTECTED = ('/frontend/setUser', '/frontend/addCartItem', '/frontend/cart', '/frontend/checkout')
HW_CONFIGS = [(256, 0, 'FaaS 256MB'), (512, 0, 'FaaS 512MB'),
...
def main():
    ...
    fig.suptitle('FaaS: Auth Overhead — Baseline vs Flash Crowd\n'
                 '(ΔP99, auth-only endpoints, excl. argon2id)', fontsize=12, fontweight='bold')
    plt.tight_layout()
    path = f"{PLOT_DIR}/delta_auth_baseline_vs_flashcrowd_faas.pdf"
