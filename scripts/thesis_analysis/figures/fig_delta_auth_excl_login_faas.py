#!/usr/bin/env python3
"""Generate delta_auth_excl_login_faas.pdf for fig:delta-auth-excl-login-faas.

FaaS: ΔAuth P99 with login vs excluding login (setUser).
Demonstrates that Manual (bcrypt) overhead is concentrated in login.
"""
import sys, os
...
ARCH = 'faas'
EPS_WITH = ('/frontend/setUser', '/frontend/addCartItem', '/frontend/cart', '/frontend/checkout')
EPS_WITHOUT = ('/frontend/addCartItem', '/frontend/cart', '/frontend/checkout')

HW_CONFIGS = [
    (256, 0, 'FaaS 256MB'), (512, 0, 'FaaS 512MB'),
    (1024, 0, 'FaaS 1024MB'), (1769, 0, 'FaaS 1769MB'),
]
...
def main():
    ...
    fig.suptitle('FaaS: Auth Overhead With vs Excluding Login\n'
                 '(ΔP99, Baseline phase, excl. argon2id)', fontsize=12, fontweight='bold')
    plt.tight_layout()
    path = f"{PLOT_DIR}/delta_auth_excl_login_faas.pdf"
