#!/usr/bin/env python3
"""Generate timeseries_faas512_algo_comparison.pdf for fig:timeseries-algo.

FaaS/512 MB: Hash algorithm comparison (None vs bcrypt vs argon2id)
over the full SCNAST protocol. P50 capped at 5000ms.
"""
import sys, os
...
def main():
    ...
    if eid_none is not None:
        datasets.append(('None', '#95a5a6', load_timeseries(eid_none)))
    if eid_bcrypt is not None:
        datasets.append(('Manual (bcrypt)', '#e74c3c', load_timeseries(eid_bcrypt)))
    if eid_argon is not None:
        datasets.append(('Manual (argon2id)', '#8e44ad', load_timeseries(eid_argon)))

    if not datasets:
        print('No FaaS/512 algo data')
        return
    ...
    ax_p50.set_ylim(0, 5000)
    ax_p50.grid(axis='y', alpha=0.35, linestyle='--', linewidth=0.5)
    ax_p50.tick_params(labelsize=10)
    ax_p50.set_title('FaaS (512 MB): Hash Algorithm Impact — bcrypt/HS256 vs argon2id/EdDSA',
                     fontweight='bold', fontsize=12, pad=24)
    ax_p50.legend(fontsize=10, loc='upper left', ncol=3, framealpha=0.9)
    ...
    plt.tight_layout()
    path = f"{PLOT_DIR}/timeseries_faas512_algo_comparison.pdf"
