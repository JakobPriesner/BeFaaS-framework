#!/usr/bin/env python3
"""Generate throughput_comparison_faas.pdf for fig:throughput-faas."""
import sys, os
...
def main():
    ...
    durations = query_df(f"""
        ...
        WHERE {EXCLUDE_SQL_BY_EXP_ID}
          AND experiment_id IN (SELECT e.id FROM experiments e WHERE e.architecture = 'faas' AND {EXCLUDE_SQL})
    ...
    arch_data = best[best['architecture'] == 'faas']
    if arch_data.empty:
        print('No FaaS data')
        return

    hw_tiers = [hw for _, _, hw in HW_TIERS_ALL['faas'] if hw in arch_data['hw'].values]
    ...
    ax.set_title('Durchsatz: FaaS', fontweight='bold', fontsize=13)
    ...
    plt.tight_layout()
    path = f"{PLOT_DIR}/throughput_comparison_faas.pdf"
