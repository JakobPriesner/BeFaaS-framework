#!/usr/bin/env python3
"""Generate timeseries_ms_s_edge_cascade.pdf for fig:timeseries-edge-cascade.

MS/S (0.25 vCPU): Lambda@Edge cascading failure — None vs Edge.
Three-panel layout: P50 (capped at 5000ms), P99 (log), error rate.
"""
import sys, os
...
AUTH_SHORT = {'none': 'None', 'edge': 'Edge'}
...
def find_experiment(configs, arch, ram, cpu, auth_strategy):
    mask = (
...
def main():
    ...
    ax_p50.set_title('Microservices (S / 0.25 vCPU): Lambda@Edge Cascading Failure',
                     fontweight='bold', fontsize=12, pad=24)
    ax_p50.legend(fontsize=10, loc='upper left', ncol=2, framealpha=0.9)
    ...
    plt.tight_layout()
    path = f"{PLOT_DIR}/timeseries_ms_s_edge_cascade.pdf"
