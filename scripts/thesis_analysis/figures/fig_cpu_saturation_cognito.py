#!/usr/bin/env python3
"""Generate cpu_saturation_cognito_combined.pdf for fig:cpu-saturation-cognito.

2-panel side-by-side: Microservices frontend-service | Monolith.
Grouped bars (mean CPU%) with p95 cap markers for None vs Cognito
during Flash Crowd phase only.
"""
import sys, os
...
def main():
    raw = load_raw()
    if raw.empty:
        print('No Cognito CPU saturation data'); return
    ...
    path = f"{PLOT_DIR}/cpu_saturation_cognito_combined.pdf"
