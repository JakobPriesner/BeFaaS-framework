#!/usr/bin/env python3
"""Generate descriptive_stats_barplots_baseline_microservices.pdf for fig:descriptive-barplots-ms.

Bar chart: mean ± StdDev of auth-only P99 per auth strategy per HW tier.
Microservices architecture, Baseline phase only.
"""
import sys, os
...
PROTECTED_ENDPOINTS = ('/setUser', '/addCartItem', '/cart', '/checkout')
...
def main():
    ...
    ax.set_title('Microservices (Fargate): Auth-only P99 (Baseline)', fontsize=13, fontweight='bold')
    ...
    plt.tight_layout()
    path = f"{PLOT_DIR}/descriptive_stats_barplots_baseline_microservices.pdf"
