#!/usr/bin/env python3
"""Generate throughput_comparison_microservices.pdf for fig:throughput-microservices."""
import sys, os
...
def main():
    df = query_df(f"""
        ...
        WHERE r.phase_name IS NOT NULL AND r.phase_name != 'None'
          AND {EXCLUDE_SQL} AND e.architecture = 'microservices'
    ...
    durations = query_df(f"""
        ...
        WHERE {EXCLUDE_SQL_BY_EXP_ID}
          AND experiment_id IN (SELECT e.id FROM experiments e WHERE e.architecture = 'microservices' AND {EXCLUDE_SQL})
    ...
    arch_data = best[best['architecture'] == 'microservices']
    if arch_data.empty:
        print('No MS data')
        return

    hw_tiers = [hw for _, _, hw in HW_TIERS_ALL['microservices'] if hw in arch_data['hw'].values]
    ...
    ax.set_title('Durchsatz: Microservices', fontweight='bold', fontsize=13)
    ...
    plt.tight_layout()
    path = f"{PLOT_DIR}/throughput_comparison_microservices.pdf"
