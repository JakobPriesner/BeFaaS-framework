#!/usr/bin/env python3
"""Generate edge_selective_delta_comparison.pdf for fig:edge-selective-delta.

Grouped bar chart showing ΔAuth P99 (relative to None) for Edge-Selective
vs Full-Edge, grouped by architecture and hardware tier.
"""
import sys, os
...
AUTH_SHORT = {
    'none': 'None', 'edge': 'Edge', 'edge-selective': 'Edge-Sel.',
}
MIN_REQUESTS = 50_000
MIN_PHASES = 7
...
def _discover_edge_selective_ids():
    ...
    df = query_df(f"""
        SELECT e.id, e.architecture, COALESCE(e.with_cloudfront, false) AS with_cf,
               e.ram_in_mb, COALESCE(e.cpu_in_vcpu, 0) AS cpu,
               COUNT(r.id) AS n_requests, COUNT(DISTINCT r.phase_name) AS n_phases
        FROM experiments e JOIN requests r ON r.experiment_id = e.id
        WHERE e.auth_strategy = 'edge-selective' AND e.id NOT IN ({excl})
        GROUP BY e.id, e.architecture, e.with_cloudfront, e.ram_in_mb, e.cpu_in_vcpu
    """)
...
def main():
    ...
    for ax, arch in zip(axes, archs_with_data):
        ...
        for i, auth in enumerate(auth_present):
            vals = []
            for hw in hw_tiers:
                auth_match = arch_data[(arch_data['auth_strategy'] == auth) & (arch_data['hw_tier'] == hw)]
                none_match = arch_data[(arch_data['auth_strategy'] == 'none') & (arch_data['hw_tier'] == hw)]
                if not auth_match.empty and not none_match.empty:
                    vals.append(auth_match.iloc[0]['P99'] - none_match.iloc[0]['P99'])
                else:
                    vals.append(0)
